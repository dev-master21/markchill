...
export const createOrder = asyncHandler(async (req: any, res: Response) => {
  ...
  if (promo_code) {
    const promoResult = await PromoService.validateAndApply(
      promo_code,
      userId,
      subtotal
    );
    ...
    const promo = await PromoService.findByCode(promo_code);
    if (promo) {
  ...
  if (promoCodeId) {
    await PromoService.recordUsage(promoCodeId, userId);
  }
